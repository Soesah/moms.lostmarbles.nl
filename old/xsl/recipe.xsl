  ...
  <xsl:template match="recipe">
		<article>
			<xsl:attribute name="class">
				<xsl:text>box recipe </xsl:text>
				<xsl:if test="$xopus">xopus</xsl:if>
			</xsl:attribute>
	...
	<xsl:template match="recipe" mode="summary">
		<section class="summary" category="{@category_id}">
			...
			<xsl:if test="$xopus or @preparation_time != ''">
	...
	<xsl:template match="@category_id" mode="text">
		<span>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<span class="category">
			      <xsl:value-of select="//categories/category[@id = current()]/@name_singular"/>
			    </span>
				</xsl:otherwise>
			</xsl:choose>
		</span>
	...
	<xsl:template match="image">
		<img class="image" src="{$path}img/recipe_large/{@source}" alt="{@source}"/>
	</xsl:template>
